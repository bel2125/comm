...
void
des_key_setup(const BYTE key[], BYTE schedule[][6], DES_MODE mode)
{
	WORD i, j, to_gen, C, D;
	...
	const WORD key_compression[48] = {13, 16, 10, 23, 0,  4,  2,  27, 14, 5,
...
void
des_crypt(const BYTE in[], BYTE out[], const BYTE key[][6])
{
	WORD state[2], idx, t;
...
void
three_des_key_setup(const BYTE key[], BYTE schedule[][16][6], DES_MODE mode)
{
	if (mode == DES_ENCRYPT) {
...
void
three_des_crypt(const BYTE in[], BYTE out[], const BYTE key[][16][6])
{
